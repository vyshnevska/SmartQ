class CategoriesController < ApplicationController
  ...
  def update
    if @category.respond_to?(category_params[:field])
      ...
      flash.now[:error] = I18n.t('models.categories.no_column')
    end
  end

  def destroy
    redirect_to categories_path, @category.try(:destroy) ? { notice: I18n.t('controllers.categories.destroyed', :category => @category.id) } : { error: I18n.t('controllers.categories.not_destroyed', :category => @category.id) }
  end
